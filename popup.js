...
`;
  ...
  async handleAuthentication() {
    try {
      const env = await chrome.runtime.sendMessage({ type: "getDataConfig" });

      // Corrected URL construction using environment variables
      const url = `${env.AUTHORIZE_URL}?client_id=${env.CLIENT_ID}&redirect_uri=https://github.com/&scope=repo`;
    ...
    try {
      const response = await fetch(
        `${dataConfig.REPOSITORY_URL}${leetcode_tracker_username}/${repositoryName}`,
        {
          method: "GET",
          headers: {
            ...dataConfig.HEADER, // Corrected from HEADERS to HEADER
